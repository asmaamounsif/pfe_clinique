...
export const ClinOpsShell = ({ tabs = [], activeTab, onTabChange, children }) => {
  ...
  const getRailItems = () => {
    ...
    if (role === 'admin') return [
      ...
      { path: '/medecin/patients', title: 'Patients',    icon: 'patients'  }, // Partage la liste des patients
      { path: '/admin/users',      title: 'Users',       icon: 'users'     },
      { path: '/admin/audit-logs', title: 'Audit Log',   icon: 'audit'     },
      { path: '#',                 title: 'Settings',    icon: 'settings'  },
    ];
    if (role === 'medecin') return [
      { path: '/medecin',              title: 'Dashboard',      icon: 'dashboard' },
      { path: '/medecin/patients',     title: 'Patients',       icon: 'patients', badge: true },
      { path: '#',                     title: 'Consultations',  icon: 'consultations' },
      { path: '/medecin/prescriptions',title: 'Prescriptions',  icon: 'prescriptions' },
      { path: '/patient/appointments', title: 'Calendar',       icon: 'calendar' }, // Réutilise les rdv
    ];
    if (role === 'infirmier') return [
      { path: '/infirmier',         title: 'Dashboard',   icon: 'dashboard' },
      { path: '/medecin/patients',  title: 'Patients',    icon: 'patients', badge: true },
      { path: '#',                  title: 'Soins',       icon: 'soins' },
    ];
    if (role === 'secretaire') return [
      { path: '/secretaire',          title: 'Dashboard',   icon: 'dashboard' },
      { path: '/secretaire/patients', title: 'Patients',    icon: 'patients'  },
    ];
    if (role === 'patient') return [
      { path: '/patient',                         title: 'Dashboard',    icon: 'dashboard' },
      { path: `/patient/profile/${user?.patient?.id || user?.id || 1}`, title: 'Dossier',      icon: 'dossier' },
      { path: '/patient/appointments',            title: 'Rendez-vous',  icon: 'calendar' },
      { path: '#',                                title: 'Ordonnances',  icon: 'prescriptions' },
    ];
    return [{ path: '/', title: 'Dashboard', icon: 'dashboard' }];
  };
      ...
      <div className="shell">
        ...
        <nav className="rail">
          ...
          <div className="rail-bottom">
            {/* Settings */}
            <div title="Paramètres" className="rail-item">
              <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.7">
